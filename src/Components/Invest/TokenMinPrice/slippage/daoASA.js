...
import config from "../slippage/config/config";
...
class DAOASATokenMinPrice {    
    static getAmountsOut = async(object) => {
        ...
        const stableAvaxVault = new web3.eth.Contract(asset.vaultAddress, asset.vaultInfo.abi);
        const stableAvaxStrategy = new web3.eth.Contract(asset.address, asset.abi);
        ...
        let USDTAmtInVault = await USDTContract.methods.balanceOf(asset.vaultAddress).call();
        USDTAmtInVault = new BigNumber(USDTAmtInVault).shiftedBy(12);
        let USDCAmtInVault = await USDCContract.balanceOf(asset.vaultAddress).call();
        USDCAmtInVault = new BigNumber(USDCAmtInVault).shiftedBy(12);
        let DAIAmtInVault = await DAIContract.balanceOf(asset.vaultAddress).call();
        ...
        if(amtWithdrawInUSD.gt(totalAmtInVault)) {
            ...
            let USDTAVAXBalance = await USDTAVAXVault.methods.balanceOf(asset.address).call();
            ...
            let USDCAVAXBalance = await USDCAVAXVault.methods.balanceOf(asset.address).call();
            ...
            let DAIAVAXBalance = await DAIAVAXVault.methods.balanceOf(asset.address).call();
