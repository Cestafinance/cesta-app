...
import { withdrawTokenThreeParam } from "../../../store/interactions/vaults";
...
function WithdrawTemplate({
  ...
  const GAEventsTracker = useGAEventsTracker("Withdrawal Approval");
  ...
  const withDrawNow = async () => {
    ...
    const status = await withdrawTokenThreeParam(
      vault.contract,
      finalSharesToWithdraw,
    ...
    if (status.success) {
      ...
      GAEventsTracker("Failure", symbol, amount);
  ...
  return (
    <Box sx={{ color: "rgb(255,255,255, 0.6)", fontFamily: "Inter" }}>
      ...
      >
        {slippageWarningNeeded && !isWithdrawing && !withdrawCompleted && (
          <Box sx={{ display: "flex" }}>
            <Box>
              <Checkbox
                sx={{
        ...
        {withdrawCompleted && (
          ...
          >
            <Box sx={{ width: "100%" }}>
              Your {symbol} has been withdrawn from {strategyInfo.name}{" "}
              successfully.
            </Box>
          </Box>
        )}
        ...
        <Box sx={{ textAlign: "center" }}>
          <DepositButton
            disabled={slippageWarningNeeded && !slippageAccepted}
            onClick={withDrawNow}
          >
            {isWithdrawing ? (
