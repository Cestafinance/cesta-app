...
import { networkIdSelector, accountSelector } from "../../store/selectors/web3";
...
function StrategyDetails({
  ...
  const getCoinDistribution = async () => {
    try {
      const distributionResponse = await getStrategyCoinDistribution(
        strategyData.performanceId
      );
        ...
        <Grid item md={6} sm={12} className={classes.paddingGrid}>
          ...
          >
            <Grid container>
              ...
              <Grid item xs={12}>
                <StrategyChart
                  {...{ strategyData, isExpanded, selectedTimeRange }}
                />
              </Grid>
            </Grid>
          </Box>
